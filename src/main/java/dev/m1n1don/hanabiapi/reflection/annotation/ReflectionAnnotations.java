package dev.m1n1don.hanabiapi.reflection.annotation;
...
import dev.m1n1don.hanabiapi.reflection.resolver.FieldResolver;
import dev.m1n1don.hanabiapi.reflection.resolver.MethodResolver;
...
public class ReflectionAnnotations
{
    ...
    public void load(Object toLoad)
    {
        ...
        for (java.lang.reflect.Field field : toLoad.getClass().getDeclaredFields())
        {
            ...
            if (classAnnotation != null)
            {
                ...
                for (int i = 0; i < names.length; i++) names[i] = names[i].replace("{nms}", "net.minecraft.server." + Minecraft.VERSION.name()).replace("{obc}", "org.bukkit.craftbukkit." + Minecraft.VERSION.name());
                try
                {
                    if (ClassWrapper.class.isAssignableFrom(field.getType())) field.set(toLoad, classResolver.resolveWrapper(names));
                    else if (java.lang.Class.class.isAssignableFrom(field.getType())) field.set(toLoad, classResolver.resolve(names));
                    else
                    {
            ...
            else if (fieldAnnotation != null)
            {
                ...
                try
                {
                    ...
                    if (FieldWrapper.class.isAssignableFrom(field.getType())) field.set(toLoad, fieldResolver.resolveWrapper(names));
                    else if (java.lang.reflect.Field.class.isAssignableFrom(field.getType())) field.set(toLoad, fieldResolver.resolve(names));
                    else
                    {
            ...
            else if (methodAnnotation != null)
            {
                ...
                for (String s : names) if (s.contains(" ") != isSignature) throw new IllegalArgumentException("Inconsistent method names: Cannot have mixed signatures/names");
                try
                {
                    ...
                    {
                        if (isSignature) field.set(toLoad, methodResolver.resolveSignatureWrapper(names));
                        else field.set(toLoad, methodResolver.resolveWrapper(names));
                    }
                    else if (java.lang.reflect.Method.class.isAssignableFrom(field.getType()))
                    {
                        if (isSignature) field.set(toLoad, methodResolver.resolveSignature(names));
                        else field.set(toLoad, methodResolver.resolve(names));
                    }
                    else
                    {
    ...
    <A extends Annotation> List<String> parseAnnotationVersions(java.lang.Class<A> clazz, A annotation)
    {
        ...
        try
        {
            ...
            if (versions.length == 0) for (String name : names) list.add(name);
            else
            {
                if (versions.length > names.length) throw new RuntimeException("versions array cannot have more elements than the names (" + clazz + ")");
                for (int i = 0; i < versions.length; i++)
                {
                    if (Minecraft.VERSION == versions[i]) list.add(names[i]);
                    else
                    {
                        if (names[i].startsWith(">") && Minecraft.VERSION.newerThan(versions[i])) list.add(names[i].substring(1));
                        else if (names[i].startsWith("<") && Minecraft.VERSION.olderThan(versions[i])) list.add(names[i].substring(1));
                    }
                }
            }
        }
    ...
    <A extends Annotation> String parseClass(java.lang.Class<A> clazz, A annotation, Object toLoad)
    {
        try
        {
            ...
            while (matcher.find())
            {
                ...
                if (ClassWrapper.class.isAssignableFrom(field.getType())) return ((ClassWrapper) field.get(toLoad)).getName();
                else if (java.lang.Class.class.isAssignableFrom(field.getType())) return ((java.lang.Class) field.get(toLoad)).getName();
