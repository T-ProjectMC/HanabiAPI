...
public class MethodWrapper<R> extends WrapperAbstract
{
    ...
    {
        try { return (R) this.method.invoke(object, args); }
    ...
    public static class MethodSignature
    {
        ...
        public static MethodSignature of(Method method, boolean fullClassNames)
        {
            ...
            if (returnType.isPrimitive()) returnTypeString = returnType.toString();
            else returnTypeString = fullClassNames ? returnType.getName() : returnType.getSimpleName();
            String methodName = method.getName();
            String[] parameterTypeStrings = new String[parameterTypes.length];
            for (int i = 0; i < parameterTypeStrings.length; i++)
            {
                if (parameterTypes[i].isPrimitive()) parameterTypeStrings[i] = parameterTypes[i].toString();
                else parameterTypeStrings[i] = fullClassNames ? parameterTypes[i].getName() : parameterTypes[i].getSimpleName();
            }
        ...
        public static MethodSignature fromString(String signatureString)
        {
            ...
            if (matcher.find())
            {
                if (matcher.groupCount() != 3) throw new IllegalArgumentException("invalid signature");
                return new MethodSignature(matcher.group(1), matcher.group(2), matcher.group(3).split(","));
            }
            else throw new IllegalArgumentException("invalid signature");
        }
        ...
        public boolean matches(MethodSignature other)
        {
            ...
            for (int i = 0; i < parameterTypes.length; i++) if (!Pattern.compile(getParameterType(i).replace("?", "\\w").replace("*", "\\w*")).matcher(other.getParameterType(i)).matches()) return false;
            return true;
        }
