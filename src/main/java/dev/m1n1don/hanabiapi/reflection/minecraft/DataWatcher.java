...
public class DataWatcher {
    ...
    public static int getValueType(Object value)
    {
        ...
        else if (value != null && value.getClass().equals(ItemStack)) type = 5;
        else if (value != null && (value.getClass().equals(ChunkCoordinates) || value.getClass().equals(BlockPosition))) type = 6;
        else if (value != null && value.getClass().equals(Vector3f)) type = 7;
        return type;
    }
    ...
    public static class V1_9
    {
        ...
        {
            if (DataWatcherItemFieldResolver == null) { DataWatcherItemFieldResolver = new FieldResolver((Class<Constructor>) DataWatcherItem); }
        ...
        {
            if (DataWatcherObjectFieldResolver == null) DataWatcherObjectFieldResolver = new FieldResolver((Class<Constructor>) DataWatcherObject);
            ...
            if (genericInterfaces.length > 0)
            {
                ...
                if (type instanceof ParameterizedType)
                {
                    ...
                    if (actualTypes.length > 0) return actualTypes[0];
                }
            }
            return null;
        }
        ...
        public enum ValueType
        {
            ...
            {
                try { this.type = new FieldResolver(nmsClassResolver.resolve(className)).resolve(fieldNames).get(null); }
                catch (Exception e) { System.err.println("[ReflectionHelper] Failed to find DataWatcherObject for " + className + " " + Arrays.toString(fieldNames)); }
            }

            ValueType(String className, int index)
            {
                try { this.type = new FieldResolver(nmsClassResolver.resolve(className)).resolveIndex(index).get(null);
                }
                catch (Exception e) { System.err.println("[ReflectionHelper] Failed to find DataWatcherObject for " + className + " #" + index); }
            }
            ...
            ValueType(String className, int index, int offset)
            {
                ...
                try
                {
                    Class<Constructor> clazz = nmsClassResolver.resolve(className);
                ...
                catch (Exception e) { System.err.println("[ReflectionHelper] Failed to find DataWatcherObject for " + className + " #" + index + " (" + firstObject + "+" + offset + ")"); }
            }
